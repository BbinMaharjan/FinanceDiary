...
export default function Reports() {
  ...
  const { data: yearlyData, isLoading: loadingYearly } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<YearlyReportData>("/reports/yearly", {
        params: { year },
      });
      return data;
    },
  ...
  const { data: categoryData = [], isLoading: loadingCategories } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<CategoryReportItem[]>(
        "/reports/categories",
  ...
  const barData =
    yearlyData?.monthlyData?.map((m: MonthlySummary) => ({
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <Skeleton.Button
          active
          style={{ height: 288, borderRadius: 12, width: "100%" }}
        />
        <Skeleton.Button
          active
          style={{ height: 288, borderRadius: 12, width: "100%" }}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {pieData.length > 0 && (
        ...
        >
          <Card
            title={<Typography.Text strong>Category Breakdown</Typography.Text>}
            style={{ borderRadius: 12 }}
          >
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
